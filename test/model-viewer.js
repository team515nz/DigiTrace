...
// קבוצת מודלים שניתן לסובב (ללא הרצפה)
...
function updateRotationDisplay(id, value) {
    ...
    if (display) {
        ...
        display.textContent = \`סיבוב \${axis}: \${value}°\`;
    }
}
...
function updateValueDisplay(id, value) {
    ...
    const display = document.getElementById(displayId);
    if (display) {
        const label = id.includes('cutY') ? 'חיתוך אנכי' :
                     id.includes('cutX') ? 'חיתוך רוחב' : 'חיתוך עומק';
...
function onMouseDown(event) {
    if (alignState.active || measureState.active || calibrateState.active) return;
    isDragging = true;
    previousMousePosition = { x: event.clientX, y: event.clientY };
}

function onMouseMove(event) {
    if (!isDragging || alignState.active || measureState.active || calibrateState.active) return;
...
function toggleMeasureMode() {
    ...
    if (measureState.active) {
        ...
        btn.textContent = '❌ בטל מדידה';
        btn.classList.add('active');
        info.classList.remove('hidden');
        document.getElementById('measureStatus').textContent = 'בחר נקודה ראשונה...';
        document.getElementById('measureDistance').textContent = '';
        clearMeasurement();
    } else {
        btn.textContent = '🎯 התחל מדידה';
...
function addMeasurePoint(point) {
    ...
    if (measureState.points.length === 1) {
        document.getElementById('measureStatus').textContent = 'בחר נקודה שנייה...';
    } else if (measureState.points.length === 2) {
        // חשב מרחק
        const distance = measureState.points[0].distanceTo(measureState.points[1]);
        const calibratedDistance = distance * calibrateState.calibrationFactor;
        ...
        // הצג מרחק
        document.getElementById('measureStatus').textContent = 'מדידה הושלמה!';
        if (calibrateState.isCalibrated) {
            document.getElementById('measureDistance').textContent = 
                \`מרחק: \${calibratedDistance.toFixed(3)} יח' (גולמי: \${distance.toFixed(3)})\`;
        } else {
            document.getElementById('measureDistance').textContent = \`מרחק: \${distance.toFixed(3)} יחידות\`;
        }
        ...
        setTimeout(() => {
            if (measureState.active) {
                clearMeasurement();
                document.getElementById('measureStatus').textContent = 'בחר נקודה ראשונה למדידה חדשה...';
                document.getElementById('measureDistance').textContent = '';
            }
        }, 3000);
    }
...
function toggleCalibrateMode() {
    calibrateState.active = !calibrateState.active;
    const btn = document.getElementById('calibrateBtn');
    const info = document.getElementById('calibrateInfo');
    ...
    if (calibrateState.active) {
        ...
        btn.textContent = '❌ בטל כיול';
        btn.classList.add('active');
        info.classList.remove('hidden');
        document.getElementById('calibrateStatus').textContent = 'בחר נקודת התחלה של מרחק ידוע...';
        document.getElementById('applyCalBtn').disabled = true;
        clearCalibrateMeasurement();
    } else {
        btn.textContent = '📐 כיול מודל';
        btn.classList.remove('active');
        info.classList.add('hidden');
        clearCalibrateMeasurement();
    }
...
function clearCalibrateMeasurement() {
    calibrateState.points = [];
    calibrateState.markers.forEach(marker => scene.remove(marker));
    calibrateState.markers = [];
    if (calibrateState.line) {
        scene.remove(calibrateState.line);
        calibrateState.line = null;
    }
    calibrateState.measuredDistance = 0;
}

function addCalibratePoint(point) {
    // הוסף סמן
    const markerGeometry = new THREE.SphereGeometry(0.1, 16, 16);
    const markerMaterial = new THREE.MeshBasicMaterial({ 
        color: calibrateState.points.length === 0 ? 0x00ff00 : 0xff00ff 
    });
    const marker = new THREE.Mesh(markerGeometry, markerMaterial);
    marker.position.copy(point);
    scene.add(marker);
    calibrateState.markers.push(marker);
    calibrateState.points.push(point);

    if (calibrateState.points.length === 1) {
        document.getElementById('calibrateStatus').textContent = 'בחר נקודת סיום של מרחק ידוע...';
    } else if (calibrateState.points.length === 2) {
        // חשב מרחק
        const distance = calibrateState.points[0].distanceTo(calibrateState.points[1]);
        calibrateState.measuredDistance = distance;
        
        // צור קו
        const lineGeometry = new THREE.BufferGeometry().setFromPoints(calibrateState.points);
        const lineMaterial = new THREE.LineBasicMaterial({ color: 0xff00ff, linewidth: 3 });
        calibrateState.line = new THREE.Line(lineGeometry, lineMaterial);
        scene.add(calibrateState.line);

        // הצג מרחק והפעל כפתור
        document.getElementById('calibrateStatus').textContent = 
            \`נמדד מרחק: \${distance.toFixed(3)} יחידות גולמיות. הזן מרחק ידוע:\`;
        document.getElementById('applyCalBtn').disabled = false;
    }
}

function applyCalibration() {
    const knownDistance = parseFloat(document.getElementById('calibrateValue').value);
    
    if (isNaN(knownDistance) || knownDistance <= 0) {
        alert('נא להזין מרחק חוקי');
        return;
    }

    if (calibrateState.measuredDistance === 0) {
        alert('נא למדוד מרחק תחילה');
        return;
    }

    // חשב גורם כיול
    calibrateState.calibrationFactor = knownDistance / calibrateState.measuredDistance;
    calibrateState.isCalibrated = true;

    // הצג תוצאה
    document.getElementById('calibrateResult').innerHTML = 
        \`<strong>✓ כיול הוחל בהצלחה!</strong><br>
        גורם כיול: \${calibrateState.calibrationFactor.toFixed(4)}\`;

    // הצג תצוגת כיול
    document.getElementById('calibrateFactorText').textContent = 
        \`גורם: \${calibrateState.calibrationFactor.toFixed(4)} (\${calibrateState.measuredDistance.toFixed(3)} → \${knownDistance})\`;
    document.getElementById('calibrateDisplay').classList.remove('hidden');

    // כבה מצב כיול אחרי 2 שניות
    setTimeout(() => {
        if (calibrateState.active) {
            toggleCalibrateMode();
        }
    ...
    // עדכן את הגרידה בהתאם לכיול
    updateGridScale();
}

function resetCalibration() {
    calibrateState.calibrationFactor = 1.0;
    calibrateState.isCalibrated = false;
    document.getElementById('calibrateDisplay').classList.add('hidden');
    document.getElementById('calibrateResult').innerHTML = '';
    document.getElementById('calibrateValue').value = '';
    updateGridScale();
}

function updateGridScale() {
    // עדכן גודל רשת בהתאם לכיול
    if (globalBoundingBox && gridHelper) {
        const size = globalBoundingBox.getSize(new THREE.Vector3());
        const maxDim = Math.max(size.x, size.y, size.z) * calibrateState.calibrationFactor;
...
function onCanvasClick(event) {
    if (!measureState.active && !alignState.active && !calibrateState.active) return;
...
function handleFileSelect(event) {
    ...
    loadingMessage.textContent = \`טוען \${files.length} קבצים...\`;
    ...
    models.forEach(model => {
        ...
        item.innerHTML = \`
            ...
            <div class="model-item-controls">
                <button onclick="toggleModelVisibility(\${model.id})" class="model-btn" title="הצג/הסתר">
                    \${model.visible ? '👁️' : '🚫'}
                </button>
                <button onclick="toggleWireframe(\${model.id})" class="model-btn" title="מסגרת תיל">
                    🔲
                </button>
                <button onclick="deleteModel(\${model.id})" class="model-btn delete-btn" title="מחק">
                    🗑️
                </button>
                <button onclick="startAlignment(\${model.id})" class="model-btn align-btn" 
                        \${models.length < 2 ? 'disabled' : ''} title="יישר למודל אחר">
                    📍
                </button>
            </div>
        \`;
...
function startAlignment(modelId) {
    if (models.length < 2) {
        alert('נדרשים לפחות 2 מודלים ליישור');
        return;
    }
...
function finishCurrentModel() {
    if (alignState.selectingModel === 1) {
        if (alignState.points1.length < alignState.minPoints) {
            alert(\`נדרשות לפחות \${alignState.minPoints} נקודות\`);
            return;
        }
...
function updateAlignmentUI() {
    ...
    counter.textContent = \`מודל 1: \${count1} נקודות | מודל 2: \${count2} נקודות\`;

    if (alignState.selectingModel === 1) {
        status.textContent = \`בחר נקודות על: \${alignState.model1.name}\`;
        finishBtn.classList.toggle('hidden', count1 < alignState.minPoints);
        undoBtn.classList.toggle('hidden', count1 === 0);
        executeBtn.disabled = true;
    } else if (alignState.selectingModel === 2) {
        status.textContent = \`בחר נקודות על: \${alignState.model2.name}\`;
...
function executeAlignment() {
    if (alignState.points1.length !== alignState.points2.length ||
        ...
        alert('מספר הנקודות לא תואם או לא מספיק נקודות');
        return;
    }
...
function getCombinedGeometry() {
    ...
    models.forEach(model => {
        ...
        // קבל מטריצה סופית שכוללת גם סיבוב הקבוצה וגם טרנספורמציה של המודל
        const finalMatrix = new THREE.Matrix4();
        finalMatrix.multiplyMatrices(modelsGroup.matrixWorld, model.mesh.matrixWorld);

        for (let i = 0; i < positions.length; i += 3) {
            const vertex = new THREE.Vector3(positions[i], positions[i + 1], positions[i + 2]);
            
            // החל טרנספורמציה (כולל סיבוב)
            vertex.applyMatrix4(finalMatrix);
            
            // החל כיול אם קיים
            if (calibrateState.isCalibrated) {
                vertex.multiplyScalar(calibrateState.calibrationFactor);
            }
            ...
            if (normals) {
                ...
                normal.transformDirection(finalMatrix);
